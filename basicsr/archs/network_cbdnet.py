...
class NACBDNet(nn.Module):
    def __init__(self, ckpt=None):
        super(NACBDNet, self).__init__()
        from .denoising_networks import caculate_factors
    ...
    def estimate_sigma(self, x, t=1):
        ...
        x = torch.mean(torch.abs(x), dim=1)
        x = x * self.factors[t]
        return x

    def forward(self, x, t=2):
        noise_level = self.estimate_sigma(x, t)[:, None, None, None]
        noise_level_map = noise_level * torch.ones_like(x)
        concat_img = torch.cat([x, noise_level_map], dim=1)
        out = self.unet(concat_img) + x
        # return noise_level, out
        return out
